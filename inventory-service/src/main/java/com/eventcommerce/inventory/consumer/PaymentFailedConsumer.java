...
public class PaymentFailedConsumer {
    ...
    public void consume(BaseEvent<PaymentFailedPayload> event) {
        ...
        if (idempotencyService.isDuplicate(eventId)) {
            log.warn("Duplicate PAYMENT_FAILED ignored eventId={}", eventId);
            return;
        }

        log.info("Processing PAYMENT_FAILED event for orderId={}", event.getPayload().getOrderId());
        
        inventoryService.releaseReservedInventory(event.getPayload().getOrderId());
